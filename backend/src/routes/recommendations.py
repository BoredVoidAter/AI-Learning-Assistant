...
def get_study_recommendations(current_user):
    ...
    try:
        ...
        user_progress = {
            ...
            "completed_learning_paths": LearningPath.query.filter_by(user_id=current_user.id, progress_percentage=100).count(),
            "total_quizzes_taken": QuizAttempt.query.filter_by(user_id=current_user.id).count(),
            "average_quiz_score": db.session.query(func.avg(QuizAttempt.percentage)).filter_by(user_id=current_user.id).scalar() or 0,
            "total_resources_completed": Resource.query.join(Topic).join(LearningPath).filter(
