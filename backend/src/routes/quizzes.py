...
from src.models.learning import db, Quiz, Question, QuizAttempt, Topic, LearningPath
...
def get_quizzes(current_user):
    ...
    try:
        ...
        for quiz in quizzes.items:
            ...
            best_attempt = QuizAttempt.query.filter(
...
def get_quiz(current_user, quiz_id):
    ...
    try:
        ...
        quiz = db.session.query(Quiz).join(Topic).join(LearningPath).filter(
            Quiz.id == quiz_id,
            LearningPath.user_id == current_user.id
        ).first()
...
def add_question_to_quiz(current_user, quiz_id):
    ...
    try:
        ...
        max_order = db.session.query(db.func.max(Question.order_index)).filter(
            Question.quiz_id == quiz_id
        ).scalar() or 0
...
def start_quiz_attempt(current_user, quiz_id):
    ...
    try:
        ...
        attempt = QuizAttempt(
            ...
            quiz_id=quiz_id,
            max_score=questions_count  # Assuming 1 point per question
        )
